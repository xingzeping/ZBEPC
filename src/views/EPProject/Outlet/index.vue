    ...
    <div class="container dataContent">
      <el-table :data="projectData" border show-summary>
        ...
        <el-table-column prop="name" label="环保站">
				...
				<el-table-column prop="address" label="地块信息">
				</el-table-column>
				<el-table-column prop="acreage" label="面积">
				</el-table-column>
				<el-table-column prop="shape" label="出水口形状">
				</el-table-column>
				<el-table-column prop="number" label="出水口数量/个">
				</el-table-column>
				<el-table-column prop="diameter" label="出水口直径">
				</el-table-column>
				<el-table-column prop="spacing" label="出水口间距/米">
				</el-table-column>
...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
			...
			// TODO
      request({
        url: `ZBEPS/outletRecord/list/10000/1`,
        method: 'post',
				headers: {
        ...
        if (response.status === 200) {
          ...
          const tHeader = ['片区', '分公司','环保站','地块信息','面积','出水口形状','出水口数量/个','出水口直径','出水口间距/米','状态','录入人','审核人','录入时间']
      		const filterVal = [ 'pq', 'fgs','name','address','acreage','shape','number','diameter','spacing','status','creatorName','auditorName','createTime1']
				  const list = dd
...
// 			this.searchForm.year= this.date.getFullYear()+"";
// 			this.searchForm.month = (this.date.getMonth()+1)+"";
			this.searchForm.createTime = this.date;
			this.searchForm.pageSize = this.pageSize;
			this.searchForm.pageNum = this.pageNum;
			// TODO
      request({
        url: `ZBEPS/outletRecord/list/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
...
//			this.searchForm.month = (this.date.getMonth()+1)+"";
      ...
      request({
        url: `ZBEPS/outletRecord/list/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
