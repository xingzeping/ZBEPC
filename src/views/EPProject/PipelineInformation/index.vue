  ...
  <div class="app-container">
    ...
    <div class="container dataContent">
      <el-table :data="projectData" border>
        ...
        <el-table-column prop="land" label="地块">
        </el-table-column>
        <el-table-column prop="acreage" label="覆盖面积/亩">
        </el-table-column>
        <el-table-column prop="place" label="地表位置">
        </el-table-column>
        <el-table-column prop="terrain" label="管网地型">
        </el-table-column>
        <el-table-column prop="function" label="管网功用">
        </el-table-column>
        <el-table-column prop="length" label="管网长度/米">
        </el-table-column>
        <el-table-column prop="model" label="管网型号">
        </el-table-column>
        <el-table-column prop="material" label="管网材质">
        </el-table-column>
        <el-table-column label="建设完成时间">
					<template slot-scope="scope">
						{{timeFormat(scope.row.completeTime,1)}}
					</template>
				</el-table-column>
...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
			...
			// TODO
      request({
        url: `ZBEPS/pipeNetworkRecord/list/10000/1`,
        method: 'post',
				headers: {
        ...
        if (response.status === 200) {
          ...
          for(var i in dd){
		          	...
		          	dd[i].completeTime1 = timeFormat(dd[i].completeTime,'yyyy-MM-dd')
		          	if(dd[i].status===0){
									dd[i].status="未审核"
								}else{
									dd[i].status="已审核"
          ...
          const tHeader = ['片区', '分公司','环保站','地块','覆盖面积/亩','地表位置','管网地型','管网功用','管网长度/米','管网型号','管网材质','建设完成时间','状态','录入人','审核人','录入时间']
      		const filterVal = [ 'pq', 'fgs','name','land','acreage','place','terrain','function','length','model','material','completeTime1','status','creatorName','auditorName','createTime1']
				  const list = dd
...
// 			this.searchForm.month = (this.date.getMonth()+1)+"";
			...
			// TODO
      request({
        url: `ZBEPS/pipeNetworkRecord/list/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
...
//			this.searchForm.month = (this.date.getMonth()+1)+"";
      ...
      request({
        url: `ZBEPS/pipeNetworkRecord/list/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
