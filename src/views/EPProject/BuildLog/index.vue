  ...
  <div class="app-container">
    ...
    <div class="container dataContent">
      <el-table :data="projectData" border show-summary>
        ...
        <el-table-column prop="address" label="地块位置">
        </el-table-column>
        <el-table-column prop="beginTime1" label="开始日期">
        </el-table-column>
        <el-table-column prop="endTime1" label="结束日期">
        </el-table-column>
        <el-table-column prop="company" label="施工单位">
        </el-table-column>
        <el-table-column prop="days" label="施工天数">
        </el-table-column>
        <el-table-column prop="acreage" label="完成面积/亩">
        </el-table-column>
        <el-table-column prop="length" label="完成管网长度/米">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
			const v = this
			v.searchForm.createTimeStr = dateFormat(this.date,'yyyy-MM');
			v.searchForm.pageSize = 10000;
			v.searchForm.pageNum = this.pageNum;
			// TODO
      request({
        url: `ZBEPS/buildRecord/list/10000/1`,
        method: 'post',
				headers: {
        ...
        if (response.status === 200) {
          ...
          for(var i in dd){
		          	dd[i].beginTime1 = timeFormat(dd[i].beginTime,'yyyy-MM-dd')
		          	dd[i].endTime1 = timeFormat(dd[i].endTime,'yyyy-MM-dd')
		          	...
		          	if(dd[i].status===0){
								...
								const tHeader = ['片区', '分公司','环保站','地块位置','开始日期','结束日期','施工单位','施工天数','完成面积/亩','完成管网长度/米','状态','录入人','审核人','录入时间']
								const filterVal = [ 'pq', 'fgs','epName','address','beginTime1','endTime1','company','days','acreage','length','status','creatorName','auditorName','createTime1']
								const list = dd
								const titlename = this.title+timeFormat(dd[0].endTime,'yyyy-MM')
...
// 			this.searchForm.month = (this.date.getMonth()+1)+"";
			this.searchForm.createTimeStr = dateFormat(this.date,'yyyy-MM')
			this.searchForm.pageSize = pageSize;
			this.searchForm.pageNum = this.pageNum;
			// TODO
      request({
        url: `ZBEPS/buildRecord/list/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
        ...
        if (response.status === 200) {
          ...
          for(var i in v.projectData){
          	v.projectData[i].beginTime1 = timeFormat(v.projectData[i].beginTime,'yyyy-MM-dd')
          	v.projectData[i].endTime1 = timeFormat(v.projectData[i].endTime,'yyyy-MM-dd')
...
//			this.searchForm.month = (this.date.getMonth()+1)+"";
      ...
      request({
        url: `ZBEPS/buildRecord/list/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
