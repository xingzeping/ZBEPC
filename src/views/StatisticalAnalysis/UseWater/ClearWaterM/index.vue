  ...
  <div class="app-container">
        ...
        <el-row :gutter="30">
          <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
            <el-form-item label="片区:" class="postInfo-container-item">
              <el-input v-model="searchForm.pq_name" placeholder="请输入片区"/>
            </el-form-item>
          </el-col>
    ...
    <div class="container dataContent">
      <el-table :data="projectData" border show-summary>
        ...
        <el-table-column prop="sanction" label="奖罚">
        </el-table-column>
				</el-table>
      <el-pagination
				...
				// TODO
			      request({
			        ...
			        if (response.status === 200) {
			          ...
			          for(var i in dd){
					        if(dd[i].quota>dd[i].use_total){
					          	dd[i].sanction=((dd[i].quota-dd[i].use_total)*1).toFixed(2)
					        	}else{
					          	dd[i].sanction=((dd[i].quota-dd[i].use_total)*2).toFixed(2)
					        }
				        }
			          const tHeader = ['片区', '分公司','环保站','本月定额','本月实际','本月偏差','奖罚']
			      	  const filterVal = [ 'pq_name', 'fgs_name','name','quota','use_total','subtract','sanction']
							  const list = dd
      ...
      request({
        ...
        if (response.status === 200) {
          ...
          for(var i in v.projectData){
		        if(v.projectData[i].quota>v.projectData[i].use_total){
		          	v.projectData[i].sanction=((v.projectData[i].quota-v.projectData[i].use_total)*1).toFixed(2)
		        	}else{
		          	v.projectData[i].sanction=((v.projectData[i].quota-v.projectData[i].use_total)*2).toFixed(2)
		        }
	        }
