...
<script>
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
		...
		// TODO
	      request({
	        url: `ZBEPS/PowerGeneration/count/10000/1`,
	        method: 'post',
			headers: {
	        ...
	        if (response.status === 200) {
	          ...
	          const tHeader = ['片区', '分公司','环保站','日期','沼气发电机功率/kw','发电时长/h','发电量/度']
	      	  const filterVal = [ 'pq', 'fgs','name','time1','generatorPower','duration','kilowatt']
			  const list = dd
    ...
    search () {
			...
			// TODO
      request({
        url: `ZBEPS/PowerGeneration/count/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
    ...
    requestPageData (page) { // 请求分页数据
      ...
      request({
        url: `ZBEPS/PowerGeneration/count/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
