  ...
  <div class="app-container">
    ...
    <div class="container dataContent">
      <el-table :data="projectData" border show-summary>
        ...
        <el-table-column label="用电区域" prop="area" />
        <el-table-column label="年月" prop="time1">
				</el-table-column>
        <el-table-column label="上月指数" prop="lastValue" />
        <el-table-column label="本月指数" prop="value" />
        <el-table-column label="用电量" prop="useValue" />
        <el-table-column label="猪场合计" prop="pigfarmSum" />
      </el-table>
      <el-pagination
...
export default {
		...
		methods: {
			...
			exportExcel() { // 导出Excel
				...
				// TODO
			      request({
			        url: `ZBEPS/electricity/count/10000/1`,
			        method: 'post',
					headers: {
			        ...
			        if (response.status === 200) {
			          ...
			          for(var i in dd){
          				...
          				dd[i].useValue = dd[i].value-dd[i].lastValue
          				dd[i].pigfarmSum = 0
		          ...
		          //计算猪场合计
								for(var a in dd){
									for(var j in dd){
										if(dd[a].orgId === dd[j].orgId){
											dd[a].pigfarmSum =dd[j].useValue+dd[a].pigfarmSum
										}
									}
								}
			          const tHeader = ['片区', '分公司','环保站','用电区域','年月','上月指数','本月指数','用电量','猪场合计']
			      	  const filterVal = [ 'pq', 'fgs','name','area','time1','lastValue','value','useValue','pigfarmSum']
							  const list = dd
			...
			search() {
				...
				request({
				  url: `ZBEPS/electricity/count/${this.pageSize}/1`,
				  method: 'post',
					headers: {
						'Content-Type': 'application/json'
					},
...
//							v.projectData.pigfarmSum+=response.data.
						}
          //计算用电量
						for(var i=0;i<v.projectData.length;i++){
							v.projectData[i].useValue = v.projectData[i].value-v.projectData[i].lastValue
							v.projectData[i].pigfarmSum = 0
							v.projectData[i].time1 =  timeFormat(v.date,'yyyy-MM');
						}
          //计算猪场合计
						for(var a=0;a<v.projectData.length;a++){
							for(var j=0;j<v.projectData.length;j++){
								if(v.projectData[a].orgId === v.projectData[j].orgId){
									v.projectData[a].pigfarmSum =v.projectData[j].useValue+v.projectData[a].pigfarmSum
								}
							}
						}
			...
			requestPageData(page) { // 请求分页数据
				...
				request({
				  url: `ZBEPS/electricity/count/${this.pageSize}/${page}`,
				  method: 'post',
					headers: {
						'Content-Type': 'application/json'
					},
