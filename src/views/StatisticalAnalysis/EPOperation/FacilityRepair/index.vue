  ...
  <div class="app-container">
    ...
    <div class="container dataContent">
      <el-table :data="projectData" border show-summary>
        ...
        <el-table-column prop="total" label="保养维修次数">
        </el-table-column>
				</el-table>
      <el-pagination
...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
				...
				// TODO
			      request({
			        url: `ZBEPS/repairRecord/count/10000/1`,
			        method: 'post',
							headers: {
			        ...
			        if (response.status === 200) {
			          ...
			          const tHeader = ['片区', '分公司','环保站','设备/设施名称','设备/设施编号','维修日期','现行状态','保养维修次数']
			      	  const filterVal = [ 'pq', 'fgs','epName','name','number','repairTime1','currentStatus','total']
							  const list = dd
    ...
    search () {
			...
			// TODO
      request({
        url: `ZBEPS/repairRecord/count/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
    ...
    requestPageData (page) { // 请求分页数据
      ...
      request({
        url: `ZBEPS/repairRecord/count/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
