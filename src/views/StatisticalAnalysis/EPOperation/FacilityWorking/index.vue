  ...
  <div class="app-container">
    ...
    <div class="container dataContent">
      <el-table :data="projectData" border show-summary >
        ...
        <el-table-column prop="runningStatus" label="运行状态">
        </el-table-column>
        <el-table-column prop="needRepair" label="维修需求">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
				...
				// TODO
			      request({
			        url: `ZBEPS/runningRecord/count/10000/1`,
			        method: 'post',
							headers: {
			        ...
			        if (response.status === 200) {
			          ...
			          const tHeader = ['片区', '分公司','环保站','设备/设施名称','设备/设施编号','运行时间/小时','运行状态','维修需求','维修需求时间']
			      	  const filterVal = [ 'pq', 'fgs','epName','name','number','runningTime','runningStatus','needRepair','repairTime1']
							  const list = dd
    ...
    search () {
			...
			// TODO
      request({
        url: `ZBEPS/runningRecord/count/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
    ...
    requestPageData (page) { // 请求分页数据
      ...
      request({
        url: `ZBEPS/runningRecord/count/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
