...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
				...
				// TODO
			      request({
			        url: `ZBEPS/maintain/count/10000/1`,
			        method: 'post',
							headers: {
			        ...
			        if (response.status === 200) {
			          ...
			          const tHeader = ['片区', '分公司','环保站','设备/设施名称','设备/设施编号','保养周期','现行状态','保养累计次数']
			      	  const filterVal = [ 'pq', 'fgs','epName','name','number','interval','currentStatus','total']
							  const list = dd
    ...
    search () {
			...
			// TODO
      request({
        url: `ZBEPS/maintain/count/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
    ...
    requestPageData (page) { // 请求分页数据
      ...
      request({
        url: `ZBEPS/maintain/count/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
