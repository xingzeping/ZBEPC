	...
	<div class="app-container">
		...
		<div class="container dataContent">
			<el-table :data="projectData" border show-summary>
				...
				<el-table-column label="作物种类" prop="cropType" />
				<el-table-column label="收获日期">
					<template slot-scope="scope">
						{{timeFormat(scope.row.harvestTime,1)}}
					</template>
				</el-table-column>
				<el-table-column label="地块类型" prop="soilType" />
				<el-table-column label="户主" prop="farmer" />
				<el-table-column label="地块位置" prop="address" />
				<el-table-column label="面积/亩" prop="acreage" />
				<el-table-column label="上年产量/公斤" prop="lastYearYield" />
				<el-table-column label="当年产量/公斤" prop="thisYearYield" />
...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
			...
			// TODO
      request({
        url: `ZBEPS/returnAmountRecord/list/10000/1`,
        method: 'post',
				headers: {
        ...
        if (response.status === 200) {
          ...
          for(var i in dd){
		    ...
		    dd[i].harvestTime1 = timeFormat(dd[i].harvestTime,'yyyy-MM-dd')
          ...
          const tHeader = ['片区', '分公司','环保站','年月','作物种类','收获日期','地块类型','户主','地块位置','面积/亩','上年产量/公斤','当年产量/公斤','状态','录入人','审核人','录入时间']
      		const filterVal = [ 'pq', 'fgs','name','time1','cropType','harvestTime1','soilType','farmer','address','acreage','lastYearYield','thisYearYield','status','creatorName','auditorName','createTime1']
				  const list = dd
    ...
    search () {
			...
			// TODO
      request({
        url: `ZBEPS/returnAmountRecord/list/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
    ...
    requestPageData (page) { // 请求分页数据
      ...
      request({
        url: `ZBEPS/returnAmountRecord/list/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
