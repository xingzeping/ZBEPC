	...
	<div class="app-container">
		...
		<div class="container dataContent">
			<el-table :data="projectData" border show-summary>
				...
				<el-table-column label="数据产生日期">
					<template slot-scope="scope">
						{{timeFormat(scope.row.dataTime,1)}}
					</template>
				</el-table-column>
				<el-table-column label="沼气发电机功率/kw" prop="generatorPower" />
				<el-table-column label="发电时长/h" prop="duration" />
				<el-table-column label="发电量/度" prop="kilowatt" />
...
<script>
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
		...
		// TODO
	      request({
	        url: `ZBEPS/PowerGeneration/list/10000/1`,
	        method: 'post',
			headers: {
	        ...
	        if (response.status === 200) {
	          ...
	          for(var i in dd){
			    ...
			    dd[i].dataTime1 = timeFormat(dd[i].dataTime,'yyyy-MM-dd')
	          ...
	          const tHeader = ['片区', '分公司','环保站','日期','数据产生日期','沼气发电机功率/kw','发电时长/h','发电量/度','状态','录入人','审核人','录入时间']
	      		const filterVal = [ 'pq', 'fgs','name','time1','dataTime1','generatorPower','duration','kilowatt','status','creatorName','auditorName','createTime1']
					  const list = dd
    ...
    search () {
			...
			// TODO
      request({
        url: `ZBEPS/PowerGeneration/list/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
    ...
    requestPageData (page) { // 请求分页数据
      ...
      request({
        url: `ZBEPS/PowerGeneration/list/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
