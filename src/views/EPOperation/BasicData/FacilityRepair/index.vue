  ...
  <div class="app-container">
    ...
    <div class="container dataContent">
      <el-table :data="projectData" border show-summary>
        ...
        <el-table-column prop="currentStatus" label="现行状态">
          <template slot-scope="scope">
            <div>{{ scope.row.currentStatus === 0 ? '正常' : '异常' }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="total" label="维修累计次数">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
			...
			// TODO
      request({
        url: `ZBEPS/repairRecord/list/10000/1`,
        method: 'post',
				headers: {
        ...
        if (response.status === 200) {
          ...
          const tHeader = ['片区', '分公司','环保站','设备/设施名称','设备/设施编号','维修日期','现行状态','维修累计次数','状态','录入人','审核人','录入时间']
      		const filterVal = [ 'pq', 'fgs','epName','name','number','repairTime1','currentStatus','total','status','creatorName','auditorName','createTime1']
				  const list = dd
    ...
    search () {
			...
			// TODO
      request({
        url: `ZBEPS/repairRecord/list/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
    ...
    requestPageData (page) { // 请求分页数据
      ...
      request({
        url: `ZBEPS/repairRecord/list/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
