...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
			...
			// TODO
      request({
        url: `ZBEPS/pigManure/list/10000/1`,
        method: 'post',
				headers: {
        ...
        if (response.status === 200) {
          ...
          const tHeader = ['片区', '分公司','环保站','年月','猪粪产生量/吨','猪粪累计量/吨','猪粪销售/转出量/吨','猪粪库存量/吨','销售单价元/吨','销售收入','状态','录入人','审核人','录入时间']
      		const filterVal = [ 'pq', 'fgs','name','time1','yield','cumulant','sales','storage','unitPrice','income','status','creatorName','auditorName','createTime1']
				  const list = dd
    ...
    search () {
			...
			// TODO
      request({
        url: `ZBEPS/pigManure/list/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
    ...
    requestPageData (page) { // 请求分页数据
      ...
      request({
        url: `ZBEPS/pigManure/list/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
