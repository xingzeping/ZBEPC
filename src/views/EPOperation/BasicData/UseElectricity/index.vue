  ...
  <div class="app-container">
    ...
    <div class="container dataContent">
      <el-table :data="projectData" border show-summary>
        ...
        <el-table-column label="电表编号" prop="number" />
        <el-table-column label="年月" prop="time" />
        <el-table-column label="电表读数" prop="value" />
        <el-table-column label="倍数" prop="multiple" />
        <el-table-column label="上期读数" prop="lastValue" />
        <el-table-column label="本期电量" prop="useValue" />
...
export default {
		...
		methods: {
			...
			exportExcel() { // 导出Excel
				...
				// TODO
			      request({
			        url: `ZBEPS/electricity/list/10000/1`,
			        method: 'post',
							headers: {
			        ...
			        if (response.status === 200) {
			          ...
			          for(var a in dd){
									dd[a].time =  timeFormat(v.date,'yyyy-MM');
							    dd[a].createTime1 = timeFormat(dd[a].createTime,'yyyy-MM-dd')
		...
		//								debugger
			          ...
			          const tHeader = ['片区', '分公司','环保站','用电区域','电表编号','年月','电表读数','倍数','上期读数','本期电量','状态','录入人','审核人','录入时间']
			      		const filterVal = [ 'pq', 'fgs','name','area','number','time','value','multiple','lastValue','useValue','status','creatorName','auditorName','createTime1']
							  const list = dd
			...
			search() {
				...
				request({
				  url: `ZBEPS/electricity/list/${this.pageSize}/1`,
				  method: 'post',
					headers: {
						'Content-Type': 'application/json'
					},
...
//							debugger
						...
						for(var a=0;a<v.projectData.length;a++){
							v.projectData[a].time =  timeFormat(v.date,'yyyy-MM');
							if(v.projectData[a].status===0){
								v.projectData[a].status="未审核"
							}else{
								v.projectData[a].status="已审核"
							}
						}
			...
			requestPageData(page) { // 请求分页数据
				...
				request({
				  url: `ZBEPS/electricity/list/${this.pageSize}/${page}`,
				  method: 'post',
					headers: {
						'Content-Type': 'application/json'
					},
