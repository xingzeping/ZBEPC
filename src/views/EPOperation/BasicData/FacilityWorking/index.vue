  ...
  <div class="app-container">
    ...
    <div class="container dataContent">
      <el-table :data="projectData" border show-summary>
        ...
        <el-table-column prop="runningTime" label="运行时间/小时">
        </el-table-column>
        <el-table-column label="运行状态">
          <template slot-scope="scope">
            <div>{{ scope.row.runningStatus === 0 ? '正常' : '异常' }}</div>
          </template>
        </el-table-column>
        <el-table-column label="维修需求">
          <template slot-scope="scope">
            <div>{{ scope.row.needRepair === 0 ? '是' : '否' }}</div>
          </template>
        </el-table-column>
        <el-table-column label="维修需求时间">
					<template slot-scope="scope">
						{{timeFormat(scope.row.repairTime,1)}}
					</template>
				</el-table-column>
...
export default {
  ...
  methods: {
    ...
    exportExcel() { // 导出Excel
			...
			// TODO
      request({
        url: `ZBEPS/runningRecord/list/10000/1`,
        method: 'post',
				headers: {
        ...
        if (response.status === 200) {
          ...
          for(var i in dd){
          			dd[i].repairTime1 = timeFormat(dd[i].repairTime,'yyyy-MM-dd')
          ...
          const tHeader = ['片区', '分公司','环保站','设备/设施名称','设备/设施编号','运行时间/小时','运行状态','维修需求','维修需求时间','状态','录入人','审核人','录入时间']
      		const filterVal = [ 'pq', 'fgs','epName','name','number','runningTime','runningStatus','needRepair','repairTime1','status','creatorName','auditorName','createTime1']
				  const list = dd
    ...
    search () {
			...
			// TODO
      request({
        url: `ZBEPS/runningRecord/list/${this.pageSize}/1`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
    ...
    requestPageData (page) { // 请求分页数据
      ...
      request({
        url: `ZBEPS/runningRecord/list/${this.pageSize}/${page}`,
        method: 'post',
				headers: {
					'Content-Type': 'application/json'
				},
